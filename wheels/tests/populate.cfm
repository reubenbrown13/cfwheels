	...
	<cfloop list="#loc.tables#" index="loc.i">
		<cfif !ListFindNoCase("cities,shops,combikeys", loc.i)>
			...
			<cfif loc.i IS "photogalleries">
				<cfquery name="loc.query" datasource="#application.wheels.dataSourceName#">
				CREATE TRIGGER bi_#loc.i# BEFORE INSERT ON #loc.i# FOR EACH ROW BEGIN SELECT #loc.seq#.nextval INTO :NEW.photogalleryid FROM dual; END;
				</cfquery>
			<cfelseif loc.i IS "photogalleryphotos">
				<cfquery name="loc.query" datasource="#application.wheels.dataSourceName#">
				CREATE TRIGGER bi_#loc.i# BEFORE INSERT ON #loc.i# FOR EACH ROW BEGIN SELECT #loc.seq#.nextval INTO :NEW.photogalleryphotoid FROM dual; END;
				</cfquery>
			<cfelse>
				<cfquery name="loc.query" datasource="#application.wheels.dataSourceName#">
				CREATE TRIGGER bi_#loc.i# BEFORE INSERT ON #loc.i# FOR EACH ROW BEGIN SELECT #loc.seq#.nextval INTO :NEW.id FROM dual; END;
				</cfquery>
			</cfif>
		</cfif>
	</cfloop>
